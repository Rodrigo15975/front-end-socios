...
import { InitialValuesArrayFormularioSector } from "./inputsSector/inputsSector";

const FormularioRegistroSector: FC<PropsInputOptional> = ({
  fieldProps,
  remove,
}) => {
  const { values } = useFormikContext<InitialValuesArrayFormularioSector>();

  const removeFormSector = (index: number) => remove && remove(index);

  return (
    <>
      {values.sectores.map((_, index) => (
        <div
...
export default FormularioRegistroSector;
