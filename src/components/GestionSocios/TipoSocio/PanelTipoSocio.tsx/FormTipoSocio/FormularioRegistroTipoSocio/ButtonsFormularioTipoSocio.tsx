...
type PropsPropsButtonsFormularioTipoSocio = {
  onClick: () => void;
};

const ButtonsFormularioTipoSocio: FC<PropsPropsButtonsFormularioTipoSocio> = ({
  onClick,
}) => {
  const { values } = useFormikContext<InitialValuesArrayFormularioTipoSocio>();

  useEffect(() => {
    if (values.socios.length === 5) {
  ...
  return (
    <>
      ...
      >
        <Button
          type="button"
          disabled={values.socios.length === 5}
...
export default ButtonsFormularioTipoSocio;
