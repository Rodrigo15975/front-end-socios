...
type PropsPropsButtonsFormularioSector = {
  onClick: () => void;
};

const ButtonsFormularioSector: FC<PropsPropsButtonsFormularioSector> = ({
  onClick,
}) => {
  const { values } = useFormikContext<InitialValuesArrayFormularioSector>();

  useEffect(() => {
    if (values.sectores.length === 5) {
      toast.warning("Máximo 5 formularios", {
        position: "top-center",
      });
    }
  }, [values.sectores.length]);
  ...
  return (
    <>
      ...
      >
        <Button
          type="button"
          disabled={values.sectores.length === 5}
...
export default ButtonsFormularioSector;
