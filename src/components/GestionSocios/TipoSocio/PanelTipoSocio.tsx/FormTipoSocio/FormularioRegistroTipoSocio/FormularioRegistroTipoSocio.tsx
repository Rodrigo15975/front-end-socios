...
import { InitialValuesArrayFormularioTipoSocio } from "./inputsTipoSocio/inputsTipoSocio";

const FormularioRegistroTipoSocio: FC<PropsInputOptional> = ({
  fieldProps,
  remove,
}) => {
  const { values } = useFormikContext<InitialValuesArrayFormularioTipoSocio>();

  const removeFormTipoSocio = (index: number) => remove && remove(index);

  return (
    <>
      {values.socios.map((_, index) => (
        <div
...
export default FormularioRegistroTipoSocio;
